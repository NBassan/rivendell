...
class RDCut : public QObject
{
 ...
 public:
  ...
  RDCut(const QString &name,bool create=false,QSqlDatabase db=QSqlDatabase());
  RDCut(unsigned cartnum,int cutnum,bool create=false,QSqlDatabase db=QSqlDatabase());
  ~RDCut();
  bool exists() const;
  bool isValid() const;
