...
void RDWaveFile::ReadTmcTag(const QString tag,const QString value)
{
  ...
  if(tag=="END") {
    wave_data->setEndType((RDWaveData::EndType)(value[0].toAscii())); /* added to.Ascii() and removed (char) for exp conversion*/
    wave_data->setMetadataFound(true);
  }
  if(tag=="TMCIREF") {
...
bool RDWaveFile::ReadEnergyFile(QString wave_file_name)
{
 ...
 // if(energy_file.readLine(str,20) <= 0)
  if(energy_file.readLine(&(str.toStdString())[0],20) <= 0) /*edited for explicit casting */
     return false;
